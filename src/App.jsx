...
function NavBar() {
  ...
  const navLinkClass = (path) => {
    ...
    return `flex items-center gap-2 px-3 py-2 rounded-lg font-medium transition-colors ${isActive
  ...
  const mobileNavLinkClass = (path) => {
    ...
    return `flex items-center gap-4 px-4 py-3 rounded-xl font-medium transition-all ${isActive
      ? 'bg-blue-50 text-blue-700 scale-[0.98]'
      : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
      }`;
  };
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
      ...
      <div 
        className={`lg:hidden fixed inset-x-0 bg-white border-b border-slate-200 shadow-2xl transition-all duration-300 ease-in-out ${
          isOpen ? 'top-16 opacity-100 visible' : 'top-12 opacity-0 invisible pointer-events-none'
        }`}
        style={{ zIndex: 40 }}
      >
        <div className="px-4 py-4 space-y-2 max-h-[calc(100vh-4rem)] overflow-y-auto bg-slate-50/50 backdrop-blur-md">
          <Link to="/" onClick={() => setIsOpen(false)} className={mobileNavLinkClass('/')}>
            <div className={`p-2 rounded-lg ${location.pathname === '/' ? 'bg-blue-100' : 'bg-white shadow-sm'}`}>
              <Fish className="h-5 w-5" />
            </div>
            <span>Pronóstico</span>
          </Link>
          <Link to="/mapa" onClick={() => setIsOpen(false)} className={mobileNavLinkClass('/mapa')}>
            <div className={`p-2 rounded-lg ${location.pathname === '/mapa' ? 'bg-blue-100' : 'bg-white shadow-sm'}`}>
              <MapPin className="h-5 w-5" />
            </div>
            <span>Mapa</span>
          </Link>
          <Link to="/calendario" onClick={() => setIsOpen(false)} className={mobileNavLinkClass('/calendario')}>
            <div className={`p-2 rounded-lg ${location.pathname === '/calendario' ? 'bg-blue-100' : 'bg-white shadow-sm'}`}>
              <Moon className="h-5 w-5" />
            </div>
            <span>Solunar</span>
          </Link>
          <Link to="/diques" onClick={() => setIsOpen(false)} className={mobileNavLinkClass('/diques')}>
             <div className={`p-2 rounded-lg ${location.pathname === '/diques' ? 'bg-blue-100' : 'bg-white shadow-sm'}`}>
          ...
          <Link to="/reglamento" onClick={() => setIsOpen(false)} className={mobileNavLinkClass('/reglamento')}>
            <div className={`p-2 rounded-lg ${location.pathname === '/reglamento' ? 'bg-blue-100' : 'bg-white shadow-sm'}`}>
              <FileText className="h-5 w-5" />
            </div>
            <span>Reglamento</span>
          </Link>
          <Link to="/blog" onClick={() => setIsOpen(false)} className={mobileNavLinkClass('/blog')}>
             <div className={`p-2 rounded-lg ${location.pathname === '/blog' ? 'bg-blue-100' : 'bg-white shadow-sm'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            ...
            <div className="flex flex-col items-center md:items-start gap-4">
              <h3 className="font-bold text-slate-900 uppercase tracking-wider text-xs">Enlaces Rápidos</h3>
              <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-center md:text-left">
                <Link to="/" className="text-slate-500 hover:text-blue-600 text-sm transition-colors">Pronóstico</Link>
                <Link to="/mapa" className="text-slate-500 hover:text-blue-600 text-sm transition-colors">Mapa</Link>
            ...
            <div className="flex flex-col items-center md:items-end gap-4">
              <h3 className="font-bold text-slate-900 uppercase tracking-wider text-xs">Contacto</h3>
              <a href="mailto:pescaappcordoba@gmail.com" className="group flex items-center gap-3 px-4 py-2 bg-slate-50 hover:bg-blue-50 border border-slate-100 hover:border-blue-100 rounded-xl text-slate-600 hover:text-blue-600 transition-all">
